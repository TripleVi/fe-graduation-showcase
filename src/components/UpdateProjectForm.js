...
const UpdateProjectForm = ({ open, handleClose, project, onUpdate }) => {
  ...
  const handleAuthorChange = (index, field, value) => {
    const updatedAuthors = Array.isArray(formData.authors)
      ? [...formData.authors]
      : []; // Đảm bảo authors là mảng
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (section === "Files") {
        ...
        if (
          formData.paragraphIndices &&
          formData.paragraphIndices.length > 0 &&
          formData.photos
        ) {
        ...
        const hasFilesToUpdate =
          ...
          Array.from(fileData.entries()).some(
            ([key, value]) => key === "avatars" && value
          );
      ...
      if (section === "Author") {
        ...
        await api.put(`/projects/${project.id}/author-group`, {
          authors: authorsData,
        });
      ...
      if (error.response && error.response.status === 409) {
        ...
        setSnackbarMessage(
          "Conflict: The title, author name, or email is already in use."
        );
      } else {
