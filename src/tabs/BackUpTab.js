    ...
    try {
      ...
      setData((prevData) => {
        const backupsChanged =
          backupsWithId.length !== prevData.backups.length ||
          !backupsWithId.every(
            (backup, idx) => backup.id === prevData.backups[idx]?.id
          );
        if (backupsChanged) {
      ...
      <Grid container spacing={3}>
        ...
        <Grid item xs={12}>
          <Button
            variant="contained"
            color="primary"
            onClick={handleBackupConfigSubmit}
          >
            Save Configuration
          </Button>
        </Grid>
              ...
              {data.backups.map((backup) => (
                <TableRow key={backup.id}>
                  ...
                  <TableCell>
                    ...
                    <IconButton
                      disabled={isRestoring}
                      onClick={() => handleDownload(backup.name, backup.id)}
                    >
                      <GetApp />
                    </IconButton>
                  </TableCell>
